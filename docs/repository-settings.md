...
`v0.1.0` 已经作为正式发布 tag。不要创建新的 GitHub Release 或 tag，除非维护者明确决定。

本次维护者已明确批准 `v0.2.0-public-preview` 作为公开预览 tag。该 tag 只表示 public preview packaging，不表示支持真实证据处理、真实 adapter、法律意见或结果承诺。

如果未来批准新 release，请以 `CHANGELOG.md` 为 release notes 基础，并保留 known limitations。
